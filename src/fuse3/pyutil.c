...
static PyObject* sftp = NULL;
PyThreadState* gMainThreadState = NULL;

int pyCall(PyObject *instance,
            const char* method,
                PyObject *args,
                PyObject *kwargs,
				PyObject *result)
{
	...
	if (pmethod == NULL) {
		debug("Can't fetch method %s", method);
	}
    if (args == NULL) {
        args = Py_BuildValue("()");   // convert arg to Python
        if (args == NULL) {
            Py_DECREF(pmethod);
            debug("Can't build arguments list for method");
        }
    } else {
    ...
    if(kwargs != NULL)
        Py_DECREF(kwargs);
	if (result == NULL) {
		debug("Error calling Importer.%s", method);
	}
...
void init_sftp()
{
	...
	//PyGILState_STATE gstate = PyGILState_Ensure();
	const char* module_name = "sftp";
	PyObject *pmodule, *pclass, *pargs;
	pmodule = PyImport_ImportModule(module_name); // fetch module
	if (pmodule == NULL) {
		debug("Can't load module %s", module_name);
		PyErr_Print();
	}
	pclass = PyObject_GetAttrString(pmodule, "SFTP"); // fetch module.class
	Py_DECREF(pmodule);
	if (pclass == NULL) {
		debug("Can't get sftp.SFTP");
		if (PyErr_Occurred())
			PyErr_Print();
	}
	pargs = Py_BuildValue("()");
	if (pargs == NULL) {
		Py_DECREF(pclass);
		debug("Can't build arguments list");
	}
	sftp = PyEval_CallObject(pclass, pargs);  // call class()
	Py_DECREF(pclass);
	Py_DECREF(pargs);
	if (sftp == NULL) {
		debug("Error calling constructor sftp.SFTP()");
	}
	//PyGILState_Release(gstate);
}
...
//        result = QString::fromUtf8(PyString_AsString(presult));
//    Py_DECREF(presult);
//    return result;
//}
//
...
//{
//    QString attribute = sl("");
//    PyObject* pattr = PyObject_GetAttrString(obj,attr);
//    if(PyString_Check(pattr)) {
//        attribute = QString::fromUtf8(PyString_AsString(pattr));
//        Py_DECREF(pattr);
//    }
//    return attribute;
//}
...
//            if(PyInt_Check(item))
//                list.append(PyInt_AsSsize_t(item));
//            Py_DECREF(item);
//        }
//    }
//    Py_DECREF(presult);
