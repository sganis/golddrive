...
int f_opendir(const char *path, struct fuse_file_info *fi)
{
	...
	sh = gd_openfile(path, FILE_ISDIR, mode, fi);
	if (!sh)
		return -1;
...
gd_handle_t * gd_openfile(const char *path, int is_dir, unsigned int mode, 
...
int f_create(const char *path, fuse_mode_t mode, struct fuse_file_info *fi)
{
	...
	sh = gd_openfile(path, FILE_ISREG, mod, fi);
...
int f_open(const char *path, struct fuse_file_info *fi)
{
	...
	sh = gd_openfile(path, FILE_ISREG, mode, fi);
...
{
	debug("%s\n", path);
	gd_handle_t* sh = (gd_handle_t*)fi_fd(fi);
	return gd_close(sh);	
}
