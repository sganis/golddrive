...
int san_read(LIBSSH2_SESSION *session, LIBSSH2_SFTP *sftp_channel,
...
int san_read_async(SOCKET sock, LIBSSH2_SESSION *session,
...
int san_stat(LIBSSH2_SFTP *sftp, const char * path,
...
int san_lstat(LIBSSH2_SFTP *sftp, const char * path,
	LIBSSH2_SFTP_ATTRIBUTES *attrs)
{
	int rc = 0;
...
int run_command_shell(LIBSSH2_SESSION *session,
	const char *cmd, char *out, char *err)
{
	LIBSSH2_CHANNEL *channel;
	int rc;
