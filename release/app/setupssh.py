...
def main(ssh, keygen, userhost, port=22):
	...
	cmd = f'echo y | "{ssh}" -p {port} -o StrictHostKeyChecking=no {userhost} "mkdir -p .ssh && echo \'{key}\' >> .ssh/authorized_keys && chmod 700 .ssh/authorized_keys"'
	print(cmd)
	os.system(cmd)
	if test(ssh, userhost, port):
